...
stats. When extracting an API, move orchestration to framework-neutral service
functions before making route handlers depend on Streamlit state.
...
| `src/subject_registry.py` | Define 12 subject keys and their paths/languages | `Subject`, `build_subject_registry`, `get_subject`, `setup_subject_folders` |
...
The executable registry contains 12 subjects: SPF Chemistry, SPF Biology,
Political Education, Philosophy, Pedagogics/Psychology, Mathematics, Physics,
History, German, French, English, and Chemistry. This differs from the 15-card
model documented by the separate frontend and must be reconciled at an API
contract boundary.
...
| File | Responsibility | Main entry points |
| --- | --- | --- |
| `src/llm_client.py` | Non-streaming Ollama chat request and result wrapper | `generate_response`, `build_ollama_payload`, `LLMResponse` |
| `src/model_runtime.py` | Reused session, model status, warmup, streaming | `verify_model_runtime`, `warm_model`, `stream_chat_tokens` |
