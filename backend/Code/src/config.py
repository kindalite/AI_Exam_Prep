...
class AppConfig:
    ...
    api_dev_student_id: str = ""
    max_upload_bytes: int = 25 * 1024 * 1024
    api_chat_timeout_seconds: int = 75
    api_generation_timeout_seconds: int = 120
    remote_llm_base_url: str = ""
    remote_llm_api_key: str = ""
    remote_llm_model: str = ""
    remote_llm_timeout_seconds: int = 120
    fallback_model_provider: str = ""

    @property
    def intelligence_model_label(self) -> str:
        """Return the text model displayed in setup screens."""
        return f"{self.model_provider}:{self.generation_model}"

    @property
    def generation_model(self) -> str:
        """Return the selected provider's configured generation model."""
        if self.model_provider.strip().lower() in {"openai_compatible", "vllm"}:
            return self.remote_llm_model
...
def load_config(project_root: Path | None = None) -> AppConfig:
    ...
    return AppConfig(
        ...
        api_dev_student_id=os.getenv("API_DEV_STUDENT_ID", ""),
        max_upload_bytes=int(os.getenv("MAX_UPLOAD_BYTES", str(25 * 1024 * 1024))),
        api_chat_timeout_seconds=int(os.getenv("API_CHAT_TIMEOUT_SECONDS", "75")),
        api_generation_timeout_seconds=int(os.getenv("API_GENERATION_TIMEOUT_SECONDS", "120")),
        ...
        remote_llm_timeout_seconds=int(os.getenv("REMOTE_LLM_TIMEOUT_SECONDS", "120")),
        fallback_model_provider=os.getenv("FALLBACK_MODEL_PROVIDER", ""),
    )
