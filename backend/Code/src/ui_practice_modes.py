...
from .services.mock_exam_service import generate_mock_exam, start_timed_mock_exam
from .services.quiz_service import generate_quiz, start_timed_quiz
    ...
    if st.button(f"Start {title}"):
        if mode == "quiz":
            response = generate_quiz(
                ...
                topic,
                difficulty,
                number_of_questions=int(count_or_points),
                call_llm=False,
            )
            session, practice, _solution = start_timed_quiz(
                user_id=user_id,
                subject=subject,
                language=language,
                topic=topic,
            ...
            response = generate_mock_exam(subject, language, int(count_or_points), difficulty, call_llm=False)
            session, practice, _solution = start_timed_mock_exam(
                user_id=user_id,
                subject=subject,
                language=language,
                topic=topic,
