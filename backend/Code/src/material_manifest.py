...
def stable_material_id(source_layer: str, subject_key: str, path: Path) -> str:
    """Return the stable public material ID shared by inventory and indexing."""
    resolved = str(path.resolve())
    digest = hashlib.sha256(
        f"{source_layer}|{subject_key}|{resolved}".encode("utf-8")
    ).hexdigest()[:16]
