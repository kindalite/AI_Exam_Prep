...
| Variables | Purpose |
| --- | --- |
| `MODEL_PROVIDER`, `OLLAMA_HOST`, `OLLAMA_MODEL`, `OLLAMA_REQUIRED_MODEL`, `OLLAMA_VISION_MODEL` | Local model identity/runtime |
...
Only Ollama behavior is implemented in the current LLM client even though
`MODEL_PROVIDER` is configurable.
...
`MODEL_PROVIDER` alone does not switch implementations. Add a provider adapter
with a result compatible with `LLMResponse`, preserve injected fake-call seams,
implement health/warm/stream behavior, keep image handling explicit, enforce
the same token budget, update setup verification, and add unit plus real-system
smoke coverage.

## 10. Extracting the planned API backend

Recommended sequence:

1. Move remaining orchestration out of `app.py` into framework-neutral services.
2. Define Pydantic request/response types matching the frontend contract.
3. Wrap existing services in thin FastAPI handlers; do not duplicate RAG logic.
4. Resolve subject-count/ID differences between repositories.
5. Replace local login assumptions with verified frontend identity and map it to
   sanitized storage scopes.
6. Make all note retrieval user-scoped before exposing multi-user endpoints.
7. Add request IDs, structured errors, timeouts, CORS, health/model status, and
   streaming cancellation.
8. Add API contract tests alongside current in-process tests.

Until this work exists, do not run `uvicorn` against this directory or describe
it as implementing `study-swiss-star/docs/API_EXPECTATIONS.md`.
